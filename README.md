...
Audio Station 是一款桌面人声处理与音轨分离工具，使用 Python、PySide6 和
...
- **AI 音轨分离**：只有一份音频时，使用 UVR MDX-Net ONNX 模型生成独立的人声与背景音轨。

垫音消除只处理能够由歌曲音源解释的内容，因此适合保留音源中不存在的现场人声、讲话、欢呼和环境声。
它不是万能的人声分离：录音削波、严重混响、不同编曲或错误音源都会影响结果。没有准确歌曲音源时，建议使用
AI 音轨分离。
