...
- 每次运行都写入按日期命名的日志文件，未处理的异常会自动打开当天日志并提示提交 Issue
...
4. 在“导出内容”中选择要保存的音轨：仅消音结果、仅垫音，或两者。垫音轨是舞台 / 现场音频
   减去消音结果，因此带着垫音在舞台上的实际音量，两轨相加即还原原始录音。
5. 试听结果后再调整强度。如果声音出现明显的抽吸感，或人声变薄，
   请降低强度，或确认歌曲音源选择是否正确。
...
| `--tracks` | `vocal`、`backing`、`both` | 导出内容，默认 `vocal`（仅消音结果）|
