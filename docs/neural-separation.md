...
如果找不到权重，程序会从 TRvlvr 的公开 UVR 模型发布页下载到指定的目录或系统应用数据目录。
下载使用 Qt 网络栈：请求由 `QNetworkAccessManager` 发出，因此会自动沿用系统代理设置、
跟随发布页到存储主机的安全跳转，并对停滞的传输设置 120 秒超时；进度来自 `downloadProgress` 信号，
取消由定时轮询触发 `abort()`。

写入使用 `QSaveFile`：数据先写进目标同目录的临时文件，SHA-256 随写入增量计算，
只有文件大小与摘要都与目录中的登记值一致，才 `commit()` 原子改名。
任何失败、校验不符或取消都调用 `cancelWriting()`，
磁盘上不会留下半成品，也不会出现“看起来存在、其实已损坏”的模型文件。

AI 页面用 `QFileSystemWatcher` 监视上述搜索目录；权重下载完成或被手动放入时，
“需要下载 / 已就绪”的标识会立即更新，无需重新进入页面。
...
程序用模型文件的 MD5 在 `src/resources/model_data.json` 中查表，读取 FFT 尺寸、频率维度、时间维度、
补偿系数和主干名称。4 个内置模型的准确摘要均登记在该表中；没有对应记录的模型会被拒绝。
...
其中 4 个平面依次为左声道实部、左声道虚部、右声道实部和右声道虚部。固定维度必须与模型规格一致，
批次维可以是模型导出的符号维度。ONNX Runtime 当前使用 CPU 执行提供程序。

## 频谱变换与分块

每个声道执行短时傅里叶变换，截取模型需要的频率范围并组装为输入张量。最低三个频率格被清零，
以符合 MDX-Net 推理流程。模型输出的实部与虚部重新组合为复频谱，补齐未预测的高频格后通过逆变换
恢复波形。
