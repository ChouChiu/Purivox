...
全部片段完成后，整场结果按舞台录音自身的采样率与位深原子写出 PCM WAV。未匹配区间本就是整场原音的
逐位副本，把导出规格抬到固定下限只会让文件变大，不会增加原录音没有的频谱细节。

若还要求导出垫音，写完消音结果后就地做整场减去渲染结果，得到第二个文件。渲染缓冲区本就是整场的
副本再逐段混入处理结果，所以差值精确等于被消掉的部分：未识别区间两者逐位相同，垫音在那里是静音——
那些区间本来就没有做过对消。片段两端 50 ms 的淡入淡出同样自洽，垫音按同一权重淡入淡出。
