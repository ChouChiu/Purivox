...
- 只有在有真实磁盘的平台上才映射。浏览器版跑在 Emscripten 上，那里的文件系统本身就在内存里，
  映射反而要多占一份，所以 `create_pcm_audio` 在那里直接在堆上分配；见[浏览器版](web.md)。
...
不会产生输入或模型里没有的频谱细节，因此不再这样做。
位深方面，8-bit 与 16-bit PCM 输入按 16-bit 写出，
...
未被任何人接住的异常由 `app/crash_handler.py` 处理：PySide6 只打印回溯并让事件循环继续，因此它是一次上报而不是退出——回溯以 CRITICAL 写入当天日志，日志文件用系统默认程序打开，对话框把用户引向 Issue 页面。链接里只带版本、平台、运行方式和异常类型——异常消息不带，它最可能是一条路径。日志不进链接：它含绝对路径，URL 在任何人读到之前就已经进了浏览器历史，GitHub 对超长 URL 返回 414 而不是截断，而一天的日志 percent-encode 之后本就超预算；它改走剪贴板，由用户自己粘进表单的代码块，粘之前看得见自己在发什么。剪贴板只在按下「提交 Issue」之后才写。日志字段不用 schema 的 `render:`——被预填的 render 字段在 GitHub 上无法编辑，而该字段本身就带 `value:` 预填；代码块因此在模板里用 `<details>` 加 ``` 围栏写死。一次运行只弹一个对话框（`paintEvent` 里的异常每帧都会重来）。原生崩溃和 `qFatal` 不经过这里，但 Qt 消息处理器已经把它们写进了同一个日志文件。
