...
export function FullStagePage({ client, ready, active, onBind }: Props) {
		...
		async (recording: PreparedFile | null, songs: PreparedFile[]) => {
			setBudget(
				recording !== null && songs.length > 0
					? await estimate(client, recording, songs)
					: null,
			);
		},
		[client],
	...
	const jobRequest = useCallback(
		() => request(stage, sources, outputName, strength, includeFragments),
		[includeFragments, outputName, sources, stage, strength],
	);
	...
	}, [analysis, client, jobRequest, start]);
